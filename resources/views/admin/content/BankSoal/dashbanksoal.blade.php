    ...
    <section class="p-4 md:ml-64 h-auto pt-20">
        ...
        <div class="p-3 sm:p-5 antialiased">
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-3">
                <!-- live search -->
                @include('layouts.dashboard.live-search', ['placeholder' => 'Search by code, session...'])
                {{-- end search --}}
                                    ...
                                    <tr class="border-b" id="baris{{ $loop->iteration }}">
                                        ...
                                        <td class="px-4 py-3 border-2 whitespace-nowrap">
                                            <ul class="lg:block py-1 text-sm"
                                                ...
                                                <li class="lg:flex">
                                                    <div class="">
                                                        <a href="{{ url('/dashAdminPartListening' . '/' . $data->id_bank) }}"
                                                ...
                                                <li class="lg:flex">
                                                    <div class="">
                                                        <a href="{{ url('/dashAdminPartReading' . '/' . $data->id_bank) }}"
            ...
            <div class="relative bg-white rounded-3xl shadow-xl border border-slate-100/50 overflow-hidden">
                ...
                <div class="p-6">
                    <form class="space-y-4 modal-form" action="{{ url('/TambahBankSoalAdmin') }}" method="POST">
                        ...
                        {{-- Time fields — hidden when Self Study --}}
                        <div id="tambah-time-group" class="space-y-4">
                            <div>
                                <label for="start_time" class="block mb-1.5 text-xs font-bold text-slate-650 uppercase tracking-wider">Start Time</label>
                ...
                <div class="relative bg-white rounded-3xl shadow-xl border border-slate-100/50 overflow-hidden">
                    ...
                    <div class="p-6">
                        <form class="space-y-4 modal-form" action="{{ url('/UpdateBankSoalAdmin') }}" method="POST">
                            ...
                            {{-- Time fields — hidden when Self Study --}}
                            <div id="update-time-group-{{ $data->id_bank }}" class="space-y-4" {{ $data->mode === 'self_study' ? 'style=display:none' : '' }}>
                                <div>
                                    <label for="start_time" class="block mb-1.5 text-xs font-bold text-slate-650 uppercase tracking-wider">Start Time</label>
                                    <input type="time" name="waktu_mulai" id="start_time"
        ...
        <div class="relative p-4 w-full max-w-sm max-h-full">
            ...
            <div class="relative p-6 text-center bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">
                ...
                <div class="flex justify-center gap-3">
                    <form class="modal-form w-full flex gap-3" action="{{ url('/DeleteBankSoalAdmin') }}" method="POST">
                        @csrf
    ...
    <script>
        ...
        (function () {
            ...
            if (modeSelect) {
                ...
                applyTambahMode(modeSelect.value); // init state
            }
        })();

        /* ── Update modals: Mode → show/hide Session & Time (per bank ID) ── */
        document.querySelectorAll('.update-mode-select').forEach(select => {
            ...
            applyUpdateMode(select.value); // init
        });
    </script>
