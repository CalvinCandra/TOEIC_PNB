<nav class="bg-white border-b border-gray-200 px-4 py-2 fixed left-0 right-0 top-0 z-50">
  <div class="flex flex-wrap justify-between items-center">
    ...
    <div class="flex items-center lg:order-2">
      ...
      <button type="button" class="flex mx-3 text-sm bg-gray-800 rounded-full md:mr-0 focus:ring-4 focus:ring-gray-300"
      ...
      <!-- Dropdown menu -->
      <div class="hidden z-50 my-4 w-56 text-base list-none bg-white divide-y divide-gray-100 shadow " id="dropdown">
        <div class="py-3 px-4">
        ...
        <ul class="py-1 text-gray-700" aria-labelledby="dropdown">
          <li>
            <a href="{{url('/')}}" class="block py-2 px-4 text-sm hover:bg-gray-100">Landing</a>
          </li>
          @if (auth()->user()->level == 'peserta')
          <li>
            <a href="{{url('/download-result')}}" target="_blank"
              class="block py-2 px-4 text-sm hover:bg-gray-100">Download Result</a>
          </li>
          @endif
          <li>
            <a href="{{url('/logout')}}" class="block py-2 px-4 text-sm font-bold hover:bg-gray-100">Sign out</a>
          </li>
        </ul>
      </div>
    </div>
